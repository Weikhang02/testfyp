...
# If user doesn't know their preferences
    ...
    if user_input:
        ...
        st.markdown(f'Your input suggests you are looking for a car with a focus on <strong>{classified_class}</strong>.', unsafe_allow_html=True)
        ...
        st.markdown(f'Here are the top 5 cars based on <strong>{category}</strong>:', unsafe_allow_html=True)
